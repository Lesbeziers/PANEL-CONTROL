...
function parseMonthContextFromSheetName(sheetName) {
  const normalizedTitle = `${sheetName ?? ""}`.trim();
  const match = normalizedTitle.match(/^([A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)\s+(\d{4})$/u);
  if (!match) {
    return null;
  }
...
function parseSheetRowsToBlockRows(sheet, monthContext) {
  const matrix = window.XLSX.utils.sheet_to_json(sheet, { header: 1, raw: false, defval: "" });
  const nonSeparatorBlockIndexes = blocks
    .map((block, index) => (block.isSeparator ? null : index))
    .filter((index) => Number.isInteger(index));

  const parsedRowsByBlockIndex = new Map();
  let blockCursor = 0;
  let currentBlockIndex = null;
  let importedRowCount = 0;

  matrix.forEach((row = []) => {
    const colA = toExcelText(row[0]);
    const colB = toExcelText(row[1]);
    const colC = toExcelText(row[2]);
    const colD = toExcelText(row[3]);
    const colE = toExcelText(row[4]);
    const colF = toExcelText(row[5]);

    const isHeaderRow = normalizeBlockLabel(colA) === "listo" && normalizeBlockLabel(colB) === "titulo";
    if (isHeaderRow) {
      return;
    }

    const hasOnlyPotentialBlockLabel = !!colA && !colB && !colC && !colD && !colE && !colF;
    if (hasOnlyPotentialBlockLabel) {
      while (blockCursor < nonSeparatorBlockIndexes.length) {
        const candidateIndex = nonSeparatorBlockIndexes[blockCursor];
        blockCursor += 1;
        ...
        const excelLabel = normalizeBlockLabel(colA);
        const appLabel = normalizeBlockLabel(candidateBlock.blockType);
        currentBlockIndex = candidateIndex;

        if (excelLabel && appLabel && excelLabel !== appLabel) {
          // El orden manda; permitimos diferencias menores de etiqueta sin bloquear importación.
        }
        return;
      }
    ...
    if (!Number.isInteger(currentBlockIndex)) {
      return;
    }

    const hasData = !!colA || !!colB || !!colC || !!colD || !!colE || !!colF;
    if (!hasData) {
      return;
    }

    const rowModel = newRowForBlock(blocks[currentBlockIndex].blockType, monthContext);
    rowModel.listo = parseCellValue("listo", colA);
    rowModel.title = parseCellValue("title", colB);
    applyDateCellValue(rowModel, "startDate", colC, { preserveRawOnInvalid: true });
    applyDateCellValue(rowModel, "endDate", colD, { preserveRawOnInvalid: true });
    rowModel.genre = parseCellValue("genre", colE);
    rowModel.id = parseCellValue("id", colF);

    const existing = parsedRowsByBlockIndex.get(currentBlockIndex) || [];
    existing.push(rowModel);
    parsedRowsByBlockIndex.set(currentBlockIndex, existing);
    importedRowCount += 1;
  });

  return { parsedRowsByBlockIndex, importedRowCount };
}

function clearRowsForMonth(monthContext) {
  blocks.forEach((block) => {
    if (block.isSeparator) {
      return;
    }

    block.rows = block.rows.filter((row) => {
      if (!row) {
        return false;
      }

      return !(row.homeMonth === monthContext.month && row.homeYear === monthContext.year);
    });
  });
}
...
async function importExcelWorkbook(file) {
  ...
  if (!window.XLSX) {
    showGridToast("No se pudo cargar el lector de Excel (XLSX).");
    return;
  }
  ...
  const monthSheets = workbook.SheetNames
    .map((name) => ({ name, context: parseMonthContextFromSheetName(name) }))
    .filter((entry) => !!entry.context);

  if (!monthSheets.length) {
    showGridToast("El Excel no contiene hojas de mes válidas (ej: ENERO 2026).");
    return;
  }

  let totalImportedRows = 0;
  monthSheets.forEach(({ name, context }) => {
    const sheet = workbook.Sheets[name];
    if (!sheet) {
      return;
    }

    clearRowsForMonth(context);
    const { parsedRowsByBlockIndex, importedRowCount } = parseSheetRowsToBlockRows(sheet, context);

    parsedRowsByBlockIndex.forEach((rowsForBlock, blockIndex) => {
      const block = blocks[blockIndex];
      if (!block || block.isSeparator) {
        return;
      }
      block.rows.push(...rowsForBlock);
    });
  ...
  renderRows();
  showGridToast(`Excel importado: ${totalImportedRows} filas cargadas en ${monthSheets.length} mes(es).`);
}

function attachImportExcelAction(root) {
